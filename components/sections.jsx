...
const WRITING = [
  ...
  },
  {
    ...
    tag: "Debugging",
    body: [
      { t: "p", text: "My synth's USB audio output has produced occasional sub-millisecond dropouts since the first build. Rare enough to ignore, and I had no evidence, just a guess (\"probably the USB buffer\"). After a large engine upgrade seemed to make them worse I sat down to actually find them. The cause turned out to sit two vendor layers below my code, in Espressif's usb_device_uac component and in TinyUSB. One fix is merged in TinyUSB; the companion PR against esp-iot-solution is open." },
      { t: "p", text: "Debugging by ear was not going to work, so I recorded the host-side capture and analyzed the WAV: 193 dropouts in 19.5 seconds, each an exact run of digital zeros in both channels, 4 to 16 samples wide, start times on a 50 ms grid - the period of my display task. That ruled out the synthesis path, since a DSP bug does not write synchronized exact zeros on a UI task's schedule. Instrumenting the producer side then ruled out my own code: render time peaked at 28% of the block budget over 38,000 blocks, no overruns, near-zero ring underruns - while the clicks continued. Every counter I owned was clean, so the loss was happening below the application." },
      { t: "p", text: "Reading the two components turned up three stacked problems. usb_device_uac sizes its mic-side FIFO at two to three USB packets, and its refill logic can never fill it past roughly one packet - below the setpoint TinyUSB's flow controller steers toward. The stream therefore runs permanently one scheduling delay away from shipping a zero-length packet, which the host records as a click. TinyUSB documents that this flow controller needs a FIFO of at least four packets and has a guard for it, but the comparison is inverted, so the guard passes on FIFOs far below its own minimum instead of falling back to plain sends. And the controller corrects fill by at most ~0.4% per frame, so even a correctly sized FIFO spends the first seconds after every stream open climbing from empty, dropping packets along the way." },
      { t: "p", text: "I fixed my own device first - vendored the component, resized the FIFO, prefilled it at stream open, verified on hardware that the dropouts were gone - then upstreamed both parts: a one-line guard fix to TinyUSB, and a PR to esp-iot-solution that sizes the FIFO from the documented constraints and adds the prefill, mirroring the buffered start the component already uses on its speaker path. The two are linked: once esp-iot-solution picks up a TinyUSB release with the corrected guard, the current FIFO size stops getting flow control at all, so the sizing there has to change one way or another." },
      { t: "links", items: [
